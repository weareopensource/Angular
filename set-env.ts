...
sysEnv = _.mapKeys(sysEnv, (_v, k) => k.replace(/FRONT_/g, '').toLowerCase().replace(/_/g, '.'));
let sysMemEnv = {};
...
fs.open('./apps/default/src/environments/environment.prod.ts', 'w', (err, fd) => {
  ...
  const envConfigFile = `export const environment = ${JSON.stringify(env, null, 2).replace(/\"([^(\")"]+)\":/g,"$1:")};`;
